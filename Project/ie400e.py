...
# DECISION VARIABLES
# x[i, j, k]: 1 if (i, j) paired on side k
x = m.addMVar((N, N, 2), vtype=GRB.BINARY, name="x")
...
m.addConstr(x.sum(axis=2).sum(axis=1) <= 1, name="row")
# at most one pairing per column (for each j)
m.addConstr(x.sum(axis=2).sum(axis=0) <= 1, name="col")

# one nucleotide cannot be in two pairings
m.addConstrs(x[i, j, 0] + x[j, i_, 0] + x[i, j, 1] + x[j, i_, 1] <= 1 for i in range(N)
...
m.addConstrs(x[i, j, k] + x[i_, j_, k] <= 1 for i in range(N)
                                            for j in range(i + 1, N)
...
# if y[i, j] = 1  then  (x[i, j, 0] = 1 and x[i+1, j-1, 0] = 1) or (x[i, j, 1] = 1 and x[i+1, j-1, 1] = 1)
m.addConstrs(2*y[i, j] <= x[i, j, 0] + x[i+1, j-1, 0] + x[i, j, 1] + x[i+1, j-1, 1] for i in range(N-1)
                                                                                    for j in range(N))
